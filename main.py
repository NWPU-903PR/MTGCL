...
if cancer in ['BRCA', 'LIHC', 'COAD', 'PRAD', 'UCEC']:
    data = torch.load('./data/data_paper/CPDB_' + cancer + '_cancer.pkl')
else:
    data = torch.load('./data/data_paper/' + cancer + '_pan_cancer.pkl')
...
if mtf_type:
    # Verify the importance of mutation type features
    mtf = {
        'Silent': {'Silent/kb'},
    ...
    features = pd.DataFrame(data.x.numpy(), columns=data.features_name)
    data.x = torch.cat((data.x[:, :48], torch.tensor(features.loc[:, mtf[mtf_type]].values).type(torch.FloatTensor)),
                       dim=1)
