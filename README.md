...
``` 
python main.py -cancer "CPDB" -model "MTGCL" -e 1900 -lr 0.001 -dropout 0.5 -dropout_edge 0.5 -pe1 0.1 -pe2 0.3 -pf1 0.1 -pf2 0.5 -t 0.5 -lamuta 0.7 -cv 5 -num 10
```
...
To verify the impact of mutation type-related features on the performance of cancer driver genes identification, this can be achieved by adjusting the value of "mtf_type". The hyperparameters corresponding to each situation can be set in the utils script:
``` 
python main.py -cancer "CPDB" -model "MTGCL" -mtf_type "Missense" -e 1900 -lr 0.001 -dropout 0.5 -dropout_edge 0.5 -pe1 0.1 -pe2 0.3 -pf1 0.1 -pf2 0.5 -t 0.5 -lamuta 0.7 -cv 5 -num 10
```
The value of “mtf_type” can be ['Silent', 'Missense', 'LOF', 'Splicing', 'Polyphen2_score', 'Compound'], corresponding to different mutation categories, and the feature attributes corresponding to each category are as follows:

        -'Silent': {'Silent/kb'}
        -'Missense': {'TotMissense', 'Missense/kb', 'Missense/silent', 'Missense/benign', 'Missense/tot', 'MissenseEntr',
                     'HiMisFreq'}
        -'LOF': {'TotLOF', 'LOF/kb', 'LOF/silent', 'LOF/benign', 'LOF/tot', 'FSEntr', 'HiFSFreq',
                'NonsenseEntr', 'HiNonsenseFreq'}
        -'Splicing': {'TotSplicing', 'Splicing/silent', 'Splicing/benign', 'Splicing/tot', 'SplicEntr', 'HiSplicFreq'}
        -'Polyphen2_score': {'Hifi/Lofi', 'Hifi/benign', 'avgPolyphen2', 'Mifi/kb', 'Damaging/kb',
                            'Damaging/benign', 'Damaging/Lofi', 'TotMifi'}
        -'Compound': {'Compound/benign', 'Compound/kB', 'Nonstop/kb', 'Inframe/kb', 'Complex/kb'}
